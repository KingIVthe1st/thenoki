...
export function BackgroundSystem() {
  return (
    ...
    >
      ...
      {/* z: 2 - Animated aurora gradient mesh */}
      <AuroraMesh />

      {/* z: 3-5 - Cloud layers with parallax */}
      <CloudLayer
      ...
      {/* z: 6 - Floating particles */}
      <ParticleField />

      {/* z: 7 - Floating light orbs */}
      <FloatingLights />

      {/* z: 8 - Cinematic vignette */}
      <VignetteOverlay />

      {/* z: 9 - Premium noise texture */}
      <NoiseOverlay />
    </div>
  );
}
...
function CloudLayer({ src, depth, opacity = 0.8, zIndex }: CloudLayerProps) {
  ...
  // Combine drift + gyroscope offset
  const totalOffsetX = driftX + gyroOffset.x;
    ...
    >
      <Image
        ...
        style={{
          ...
          transform: `scale(${1.1 + (3 - depth) * 0.05}) translateY(${totalOffsetY}px)`,
        }}
        priority={depth === 2}
      />
    </motion.div>
...
function ParticleField() {
  ...
  useEffect(() => {
    ...
    const colors = {
      heart: ["#ff69b4", "#ff6b9d", "#ec4899"],
      star: ["#ffffff", "#fef3c7", "#ddd6fe"],
      sparkle: ["#ffffff", "#c4b5fd", "#fbcfe8"],
      circle: ["rgba(255,255,255,0.6)", "rgba(196,181,253,0.5)"],
    };
    ...
    const generated = Array.from({ length: 20 }, (_, i) => {
      ...
      return {
        ...
        x: Math.random() * 100,
        y: Math.random() * 100,
        size: type === "heart" ? 12 + Math.random() * 8 : 6 + Math.random() * 6,
        type,
        delay: Math.random() * 5,
      ...
      {lights.map((light) => (
        <motion.div
          ...
          style={{
            ...
            boxShadow: `0 0 ${light.size * 3}px rgba(255,255,255,0.5)`,
          }}
          animate={{
            y: [0, -60, -120, -60, 0],
